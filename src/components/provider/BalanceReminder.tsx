...
const reminderOptions = [
  {
    id: "after",
    label: "After appointment completed",
    description: "Send reminder once service is complete",
  },
  {
    id: "24h",
    label: "24 Hours Before",
    description: "Best for last-minute confirmations",
  },
  {
    id: "3d",
    label: "3 Days Before",
    description: "Recommended for most services",
  },
  {
    id: "1w",
    label: "1 Week Before",
    description: "Good for standard appointments",
  },
  {
    id: "2w",
    label: "2 Weeks Before",
    description: "Ideal for premium services",
  },
  {
    id: "1m",
    label: "1 Month Before",
    description: "Perfect for major events",
  },
];
...
export default function BalanceReminder() {
  ...
  const selectedOption = reminderOptions.find(
    (option) => option.id === selectedTiming
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!response.success) {
        throw new Error(
          response.error || "Failed to save balance reminder settings"
        );
      }

      console.log(
        "Balance reminder settings saved successfully:",
        response.data
      );
                ...
                className={`w-full p-4 text-left hover:bg-gray-50 dark:hover:bg-white/10 transition-colors
                  ${
                    selectedTiming === option.id
                      ? "bg-primary-gold/10 dark:bg-white/10"
                      : ""
                  }`}
              >
                <div className="font-medium text-gray-900 dark:text-white">
