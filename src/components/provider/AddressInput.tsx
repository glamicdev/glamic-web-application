...
import { initGoogleMaps, searchPlaces } from "../../services/places";
...
export default function AddressInput() {
  ...
  const [sameAsBilling, setSameAsBilling] = useState(
    state.serviceLocation.sameAsBilling ?? true
  );
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [isInitialized, setIsInitialized] = useState(false);
  const [isAddressSelected, setIsAddressSelected] = useState(false);
  const [isBillingAddressSelected, setIsBillingAddressSelected] =
    useState(false);
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setError(
          "Address search is temporarily unavailable. Please try again later or contact support if the issue persists."
        );
  ...
  return (
    <Layout maxWidth="lg">
      <Heading className="text-center mb-4">Enter Your Base Address</Heading>

      <Text className="text-center mb-12 max-w-xl mx-auto">
        Add your studio location or starting point for mobile services. This
        helps us calculate travel distances accurately.
      </Text>
      ...
      <motion.div {...staggerChildren} className="space-y-4">
        ...
        <div className="mt-6">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-gray-700 dark:text-gray-300">
              Billing address is the same as base address
            </span>
          </label>
        </div>
        ...
        {!sameAsBilling && (
          <motion.div
            ...
            className="mt-8"
          >
            <Text className="font-medium mb-4">Billing Address</Text>
            <div className="relative">
              <div className="relative">
                <input
                  type="text"
      ...
      >
        <Button
          ...
          disabled={
            !isAddressSelected ||
            (!sameAsBilling && !isBillingAddressSelected) ||
            loading
          }
          className="w-full"
        >
