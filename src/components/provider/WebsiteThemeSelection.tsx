...
function ThemePreview({ name, preview, selected }: ThemePreviewProps) {
  return (
    ...
    >
      <div
        className={`aspect-[16/9] rounded-2xl overflow-hidden mb-6 transition-all duration-300 relative
        ${
          selected
...
export default function WebsiteThemeSelection() {
  ...
  return (
    <motion.div {...fadeIn} className="w-full max-w-4xl mx-auto">
      ...
      <motion.div
        ...
        onClick={(e: React.MouseEvent) => {
          e.preventDefault();
          setSelectedTheme(
            selectedTheme === currentTheme.id ? "" : currentTheme.id
          );
        }}
        {...swipeHandlers}
      >
