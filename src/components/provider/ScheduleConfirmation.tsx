...
const defaultWorkDays: WorkDay[] = [
  {
    day: "Monday",
    enabled: true,
    hours: [{ id: "1", startTime: "10:00", endTime: "20:00" }],
  },
  {
    day: "Tuesday",
    enabled: true,
    hours: [{ id: "2", startTime: "10:00", endTime: "20:00" }],
  },
  {
    day: "Wednesday",
    enabled: true,
    hours: [{ id: "3", startTime: "10:00", endTime: "20:00" }],
  },
  {
    day: "Thursday",
    enabled: true,
    hours: [{ id: "4", startTime: "10:00", endTime: "20:00" }],
  },
  {
    day: "Friday",
    enabled: true,
    hours: [{ id: "5", startTime: "10:00", endTime: "20:00" }],
  },
  {
    day: "Saturday",
    enabled: true,
    hours: [{ id: "6", startTime: "10:00", endTime: "20:00" }],
  },
  ...
  const handleToggle = () => {
    if (!day.enabled) {
      onUpdate({
        ...
        enabled: true,
        hours: [
          { id: crypto.randomUUID(), startTime: "10:00", endTime: "20:00" },
        ],
      });
    } else {
    ...
    >
      ...
      {day.enabled ? (
        <div className="space-y-4">
          {day.hours.map((slot, index) => (
            <div
              key={slot.id}
              className="flex flex-wrap sm:flex-nowrap items-center gap-3 w-full"
            >
      ...
      {showCopyDropdown && canCopyTo.length > 0 && (
        <motion.div
          ...
          } top-16 mt-2 bg-white dark:bg-primary-navy/95 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 z-10 backdrop-blur-sm`}
        >
                ...
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      ...
                      setSelectedDays(
                        selectedDays.filter((d) => d !== targetDay.day)
                      );
                    }
                  }}
...
export default function ScheduleConfirmation() {
  ...
  const [scheduleSettings, setScheduleSettings] = useState<ScheduleSettings>({
    mobile:
      state.serviceLocation.type === "mobile" ||
      state.serviceLocation.type === "both"
        ? {
            workDays: state.serviceLocation.scheduleSettings?.mobile
              ?.workDays || [...defaultWorkDays],
          }
        : undefined,
    studio:
      state.serviceLocation.type === "studio" ||
      state.serviceLocation.type === "both"
        ? {
            workDays: state.serviceLocation.scheduleSettings?.studio
              ?.workDays || [...defaultWorkDays],
          }
        : undefined,
  });
      ...
      <div className="text-center mb-8">
        ...
        <div className="flex items-center gap-2 justify-center mt-4">
          ...
          <Text className="text-gray-600">
            Set your {activeType} service hours
          </Text>
        </div>
      </div>

      <motion.div variants={staggerChildren} className="grid gap-2 mb-12">
        {currentSchedule.workDays.map((day, index) => (
          <DayCard
            key={day.day}
            day={day}
            onUpdate={(updatedDay) =>
              handleDayUpdate(activeType, index, updatedDay)
            }
            canCopyTo={currentSchedule.workDays.filter(
              (d, i) =>
                i !== index &&
                (!d.enabled ||
            ...
            onCopyTo={(targetDays) =>
              handleCopyTo(activeType, index, targetDays)
            }
          />
        ))}
      </motion.div>
