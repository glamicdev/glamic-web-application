...
export default function ServicesSelection() {
  ...
  const hasSelectedServices = state.services.some(
    (service) => service.selected
  );

  const services = [
    { id: "1", name: "Makeup", icon: <Palette className="w-8 h-8" /> },
    { id: "2", name: "Nails", icon: <Scissors className="w-8 h-8" /> },
    { id: "3", name: "Spray Tan", icon: <Droplets className="w-8 h-8" /> },
    { id: "4", name: "Hair", icon: <Scissors className="w-8 h-8" /> },
    { id: "5", name: "Waxing", icon: <Bath className="w-8 h-8" /> },
    { id: "6", name: "Esthetics", icon: <User className="w-8 h-8" /> },
    { id: "7", name: "Henna", icon: <Sparkles className="w-8 h-8" /> },
    { id: "8", name: "Eyelashes", icon: <Eye className="w-8 h-8" /> },
    { id: "9", name: "Eyelashes Eyebrows", icon: <Eye className="w-8 h-8" /> },
    { id: "10", name: "Hairstyling", icon: <Scissors className="w-8 h-8" /> },
    { id: "11", name: "Barber", icon: <Scissors className="w-8 h-8" /> },
    { id: "12", name: "Wedding", icon: <Crown className="w-8 h-8" /> },
  ];
  ...
  return (
    <Layout maxWidth="xl">
      ...
      <Text className="text-center mb-16 max-w-2xl mx-auto dark:text-gray-300">
        Select the services you offer. You can always add or remove services
        later.
      </Text>
      ...
      >
        {services.map((service) => (
          <ServiceCard
            ...
            name={service.name}
            selected={
              state.services.find((s) => s.id === service.id)?.selected || false
            }
      ...
      <motion.div
        ...
        animate={{
          opacity: hasSelectedServices ? 1 : 0,
          y: hasSelectedServices ? 0 : 20,
        }}
        className="space-y-4"
      >
        <Button
