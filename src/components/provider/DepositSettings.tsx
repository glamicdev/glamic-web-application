...
function DepositOption({
  ...
  if (isCustom && selected) {
    return (
      <motion.div variants={fadeIn} className="w-24 h-24 relative">
        <input
          type="text"
          value={customValue}
...
export default function DepositSettings() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await saveDepositSettings({
        depositPercentage: selectedPercentage,
      });
    ...
    <Layout maxWidth="md">
      <div className="text-center mb-8">
        ...
        <Text className="max-w-md mx-auto">
          Set the deposit percentage required for bookings. This amount will be
          collected upfront when clients book your services.
        </Text>
      </div>
